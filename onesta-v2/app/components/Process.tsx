...
const steps = [
  { num: '01', title: 'Diagnóstico inicial', body: 'Reunión sin costo para entender tu empresa, sector y exposición real a riesgos de compliance. Sin compromiso, sin formularios interminables.' },
  { num: '02', title: 'Análisis y Matriz de Riesgo', body: 'Entrevistas en profundidad, revisión documental y mapeo de los riesgos específicos de tu organización según su actividad y estructura.' },
  { num: '03', title: 'Diseño del programa', body: 'Elaboramos el programa de integridad a medida: código de ética, políticas internas, protocolos y canales de denuncia alineados a tu cultura.' },
  { num: '04', title: 'Implementación y acompañamiento', body: 'Capacitamos a tu equipo, instalamos el programa y quedamos disponibles como soporte externo permanente. No te dejamos solo en la etapa más crítica.' },
]
    ...
    <section id="process" style={{ background: '#F4F7FA', padding: isMobile ? '80px 0' : '120px 0', color: '#0D1B2A' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: isMobile ? '0 24px' : '0 60px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 48 : 100, alignItems: 'start' }}>
          ...
          <div ref={leftRef} className={`reveal${leftVisible ? ' visible' : ''}`} style={{ position: isMobile ? 'relative' : 'sticky', top: isMobile ? 'auto' : 100 }}>
          ...
          <div ref={rightRef} className={`reveal reveal-delay-2${rightVisible ? ' visible' : ''}`} style={{ position: 'relative' }}>
            ...
            {steps.map(step => (
              <div key={step.num} style={{ display: 'flex', gap: 24, paddingBottom: 40, position: 'relative' }}>
                <div style={{ width: 40, height: 40, minWidth: 40, background: '#FFFFFF', border: '2px solid #1D72B8', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.72rem', fontWeight: 800, color: '#1D72B8', position: 'relative', zIndex: 1, flexShrink: 0 }}>{step.num}</div>
